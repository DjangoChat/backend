...
def create_stripe_customuser(name, email, phone):
    try:
        ...
        logger.error(f"Stripe connection error creating customer: {str(e)}")
        raise
...
def create_stripe_product(name):
    try:
        ...
        logger.error(f"Stripe connection error creating product: {str(e)}")
        raise
    except APIError as e:
        logger.error(f"Stripe API error creating product: {str(e)}")
        raise
...
def create_stripe_price(currency, unit_amount, months, product_id):
    try:
        ...
        logger.error(f"Stripe connection error creating price: {str(e)}")
        raise
    except APIError as e:
        logger.error(f"Stripe API error creating price: {str(e)}")
        raise
...
def create_stripe_checkout_session(
    ...
    try:
        ...
        logger.error(f"Stripe connection error creating checkout session: {str(e)}")
        raise
    except APIError as e:
